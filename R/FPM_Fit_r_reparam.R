...
#'
#' @export
FPM_Fit_r_reparam <- function(data, parameters, timeVar, intakeVar, Emax, hessian = FALSE)
{

  ##original r fit
  fit <- stats::optim(par = c(parameters[1], parameters[2]), fn = FPM_n2ll_r_reparam, data = data, Emax = Emax, time = timeVar, intake = intakeVar, hessian = hessian)

  fit_check <- stats::optim(par = c(fit$par[1], fit$par[2]), fn = FPM_n2ll_r_reparam, data = data, Emax = Emax, time = timeVar, intake = intakeVar, hessian = hessian)
