#' simBitesExp: Generates a bite dataset by randomly sampling time from a exponential decay distribution
#'
#' This function generates a bite dataset from meal duration, number of bites, and total intake (Emax) by randomly sampling a exponential decay distribution
#'
...
simBitesExp <- function(mealdur, nBites, Emax, id) {
  ...
  for (p in 1:nrow(data)) {
    # get randomly generated values from exponential distribution
    randexp <- round(sort(stats::rexp(data[p, "nBites"], rate = 0.5)), 2)
    rand_time <- (randexp / max(randexp)) * data[p, "mealdur"]
